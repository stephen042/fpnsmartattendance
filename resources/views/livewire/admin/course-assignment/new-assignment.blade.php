<div>
    <div>
        {{-- Card 1: New Course Assignment Form --}}
        <flux:card class="bg-zinc-50/80 dark:bg-zinc-900/50 border-zinc-200/60 shadow-sm" x-data="{
            ...
            <div class="grid grid-cols-1 md:grid-cols-6 gap-6 items-end bg-white/50 dark:bg-white/[0.02] p-4 rounded-xl border border-zinc-200/50">
                ...
                {{-- Selected Courses Pills & Selection --}}
                <div class="md:col-span-3">
                    <div class="flex flex-wrap gap-2 mb-3" x-show="selectedCourses.length">
                        <template x-for="courseId in selectedCourses" :key="courseId">
                            <span class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs bg-zinc-200 dark:bg-zinc-800 cursor-pointer"
                                  @click="removeCourse(courseId)">
                                <span x-text="[...$refs.courseSelect.options].find(o => o.value == courseId)?.text"></span>
                                ✕
                            </span>
                        </template>
                    </div>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                ...
                <div class="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
                    ...
                    <div class="w-full sm:w-48">
                        <flux:select wire:model.live="semester_id" size="sm" placeholder="All Semesters">
                                ...
                                <flux:table.cell class="p-0">
                                    <div class="w-full divide-y divide-zinc-100 dark:divide-zinc-800/60">
                                        @foreach ($lecturer->assignedCourses as $assignment)
                                            <div class="flex items-center justify-between py-2.5 px-4 hover:bg-zinc-50/50 dark:hover:bg-zinc-800/20 text-sm">

                                                <div class="flex items-center gap-3 min-w-0">
                                                    <span class="font-mono font-bold text-zinc-500 dark:text-zinc-400 w-16 shrink-0">
                                                ...
                                                <div class="flex items-center gap-4 shrink-0 text-xs font-medium text-zinc-500 dark:text-zinc-400">
                                                    {{-- Level --}}
                                                    ...
                                                    <span class="w-24 text-center">
                                                        <flux:badge color="indigo" size="sm">
                                                            {{ $assignment->course->semester?->name ?? 'N/A' }}
                                                        </flux:badge>
                                                    </span>
