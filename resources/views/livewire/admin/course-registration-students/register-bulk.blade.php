<div>
    ...
    @if (count($rows))
        <flux:card
            class="p-0 overflow-hidden bg-zinc-50/80 dark:bg-zinc-900/50 border-zinc-200/60 shadow-sm backdrop-blur-md mt-6">
            <div
                ...
                <div>
                    ...
                    <flux:subheading>Verify course details before triggering queued background inserts.
                    </flux:subheading>
                </div>
            ...
            <div class="overflow-x-auto px-1">
                <table class="w-full text-left text-xs text-zinc-600 dark:text-zinc-300 min-w-[1200px]">
                    <thead
                        class="bg-zinc-100/70 dark:bg-zinc-800/50 border-b uppercase text-[11px] font-semibold text-zinc-500">
                        <tr>
                            ...
                            <th class="p-4">Course Code(s)</th>
                            <th class="p-4 text-center">Action</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-zinc-200/60 dark:divide-zinc-800">
                        @foreach ($this->paginatedRows as $index => $row)
                            @php $realIndex = ($page - 1) * $perPage + $index; @endphp
                            <tr class="hover:bg-zinc-100/50 dark:hover:bg-white/5 transition-colors">
                                <td class="p-3">
                                    <input type="text" wire:model="rows.{{ $realIndex }}.application_number"
                                        class="w-full bg-transparent border border-zinc-200 rounded px-2 py-1 focus:ring-1 focus:ring-blue-500">
                                </td>
                                <td class="p-3">
                                    <input type="text" wire:model="rows.{{ $realIndex }}.level"
                                        class="w-full bg-transparent border border-zinc-200 rounded px-2 py-1 focus:ring-1 focus:ring-blue-500">
                                </td>
                                <td class="p-3">
                                    <input type="text" wire:model="rows.{{ $realIndex }}.course_option"
                                        class="w-full bg-transparent border border-zinc-200 rounded px-2 py-1 focus:ring-1 focus:ring-blue-500">
                                </td>
                                <td class="p-3">
                                    <input type="text" wire:model="rows.{{ $realIndex }}.academic_session"
                                        class="w-full bg-transparent border border-zinc-200 rounded px-2 py-1 focus:ring-1 focus:ring-blue-500">
                                </td>
                                <td class="p-3">
                                    <input type="text" wire:model="rows.{{ $realIndex }}.semester"
                                        class="w-full bg-transparent border border-zinc-200 rounded px-2 py-1 focus:ring-1 focus:ring-blue-500">
                                </td>
                                <td class="p-3">
                                    <input type="text" wire:model="rows.{{ $realIndex }}.programme"
                                        class="w-full bg-transparent border border-zinc-200 rounded px-2 py-1 focus:ring-1 focus:ring-blue-500">
                                </td>
                                <td class="p-3">
                                    <input type="text" wire:model="rows.{{ $realIndex }}.course_codes"
                                        class="w-full bg-transparent border border-zinc-200 rounded px-2 py-1 focus:ring-1 focus:ring-blue-500">
                                </td>
                                <td class="p-3 text-center">
                                    <flux:button wire:click="removeRow({{ $realIndex }})" variant="ghost"
                                        color="red" icon="x-mark" size="sm" />
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
            ...
            {{-- Pagination Footer --}}
            <div
                class="p-4 bg-zinc-100/50 dark:bg-white/5 border-t border-zinc-200/50 flex justify-between items-center text-xs text-zinc-500">
                <p>
                    Showing <span
                        class="font-bold text-zinc-800 dark:text-zinc-200">{{ count($rows) ? ($page - 1) * $perPage + 1 : 0 }}</span>
                    to <span
                        class="font-bold text-zinc-800 dark:text-zinc-200">{{ min($page * $perPage, count($rows)) }}</span>
