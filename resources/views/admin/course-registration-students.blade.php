<x-layouts::app :title="__('Student Course Enrollment')">
    <div class="space-y-8 max-w-7xl mx-auto pb-10">

        <div class="flex items-center justify-between">
            <div>
                <flux:heading size="xl">Student Course Enrollment</flux:heading>
                <flux:subheading>Register students into courses for active academic sessions and semesters.
                </flux:subheading>
            </div>
            <flux:button icon="arrow-down-tray" variant="outline" size="sm">
                Download CSV Template
            </flux:button>
        </div>

        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">

            <div class="lg:col-span-2 space-y-8">
                <flux:card class="space-y-6">
                    <div>
                        <flux:heading size="lg">Enroll Single Student into Course</flux:heading>
                        <flux:subheading>Select academic scopes and link a student to their registered course module.
                        </flux:subheading>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                        <flux:select label="Student *" placeholder="Select Student (student_id)">
                            <option value="1">John Doe (CS/2023/001)</option>
                            <option value="2">Jane Smith (CS/2023/002)</option>
                        </flux:select>

                        <flux:select label="Course / Module *" placeholder="Select Course (course_id)">
                            <option value="1">Backend Development 2 (SWD413)</option>
                            <option value="2">Data Structures (SWD416)</option>
                            <option value="3">Cloud Architecture (CLC412)</option>
                        </flux:select>

                        <flux:select label="Academic Session *" placeholder="Choose Session (academic_session_id)">
                            <option value="1">2025/2026 Academic Session</option>
                            <option value="2">2026/2027 Academic Session</option>
                        </flux:select>

                        <flux:select label="Semester *" placeholder="Choose Semester (semester_id)">
                            <option value="1">First Semester</option>
                            <option value="2">Second Semester</option>
                        </flux:select>
                    </div>

                    <div class="flex">
                        <flux:button variant="primary" icon="plus" class="px-8">
                            Save Course Registration
                        </flux:button>
                    </div>
                </flux:card>
            </div>

            <div class="space-y-8">
                <flux:card class="bg-zinc-50 dark:bg-white/5 border-dashed border-2">
                    <div class="space-y-4">
                        <div>
                            <flux:heading size="lg">Bulk Course Enrollment</flux:heading>
                            <flux:subheading>Upload a CSV mapped with foreign keys.</flux:subheading>
                        </div>

                        <div
                            class="group relative flex flex-col items-center justify-center p-6 border-2 border-dashed border-zinc-300 dark:border-zinc-700 rounded-xl hover:border-blue-500 transition-colors cursor-pointer">
                            <flux:icon name="document-arrow-up" class="mb-2 text-zinc-400 group-hover:text-blue-500" />
                            <span class="text-xs text-zinc-500 text-center">Drag and drop CSV or click to browse</span>
                            <input type="file" class="absolute inset-0 opacity-0 cursor-pointer">
                        </div>

                        <flux:button variant="filled" class="w-full">
                            Upload & Process CSV
                        </flux:button>
                    </div>
                </flux:card>
            </div>
        </div>

        <flux:card
            class="p-0 overflow-hidden bg-zinc-50/80 dark:bg-zinc-900/50 border-zinc-200/60 shadow-sm backdrop-blur-md">
            <div
                class="p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-zinc-200/50 dark:border-white/5">
                <div>
                    <div class="flex items-center gap-2">
                        <flux:heading size="lg">Review Registration Queue</flux:heading>
                        <flux:badge color="orange" size="sm">Draft Mode</flux:badge>
                    </div>
                    <flux:subheading>Verify schema keys and integrity before executing raw batch transactions.
                    </flux:subheading>
                </div>

                <div class="flex items-center gap-2 w-full md:w-auto">
                    <flux:button variant="ghost" color="red" icon="trash" size="sm">
                        Clear Queue
                    </flux:button>
                    <flux:button variant="primary" icon="plus" class="px-6">
                        Finalize & Process Registration
                    </flux:button>
                </div>
            </div>

            <div class="overflow-x-auto px-1">
                <flux:table class="min-w-[1100px] px-6">
                    <flux:table.columns>
                        <flux:table.column align="left" class="px-6">Student ID / Name</flux:table.column>
                        <flux:table.column align="left" class="px-6">Course Module</flux:table.column>
                        <flux:table.column align="left" class="px-6">Academic Session</flux:table.column>
                        <flux:table.column align="left" class="px-6">Semester</flux:table.column>
                        <flux:table.column align="center" class="px-6">Action</flux:table.column>
                    </flux:table.columns>

                    <flux:table.rows>
                        <flux:table.row class="group hover:bg-zinc-100/50 dark:hover:bg-white/5 transition-colors">
                            <flux:table.cell class="px-6 py-4">
                                <input type="text" value="John Doe (CS/2023/001)"
                                    class="w-full bg-transparent border-none focus:ring-2 focus:ring-blue-500/20 rounded px-2 py-1 text-sm font-medium text-zinc-800 dark:text-zinc-200">
                            </flux:table.cell>

                            <flux:table.cell class="px-6 py-4">
                                <select
                                    class="w-full bg-transparent border-none text-sm focus:ring-2 focus:ring-blue-500/20 rounded cursor-pointer">
                                    <option selected value="1">Backend Development 2 (SWD413)</option>
                                </select>
                            </flux:table.cell>

                            <flux:table.cell class="px-6 py-4">
                                <select
                                    class="w-full bg-transparent border-none text-sm focus:ring-2 focus:ring-blue-500/20 rounded cursor-pointer">
                                    <option selected value="1">2025/2026</option>
                                </select>
                            </flux:table.cell>

                            <flux:table.cell class="px-6 py-4">
                                <select
                                    class="w-full bg-transparent border-none text-sm focus:ring-2 focus:ring-blue-500/20 rounded cursor-pointer">
                                    <option selected value="1">First Semester</option>
                                </select>
                            </flux:table.cell>

                            <flux:table.cell align="center" class="px-6 py-4">
                                <div
                                    class="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <flux:button variant="ghost" icon="check" size="sm" class="text-green-600" />
                                    <flux:button variant="ghost" color="red" icon="x-mark" size="sm" />
                                </div>
                            </flux:table.cell>
                        </flux:table.row>

                        <flux:table.row class="bg-red-50/30 dark:bg-red-900/10 group transition-colors">
                            <flux:table.cell class="px-6 py-4">
                                <input type="text" value="Jane Smith (CS/2023/002)"
                                    class="w-full bg-transparent border-none focus:ring-2 focus:ring-blue-500/20 rounded px-2 py-1 text-sm font-medium text-zinc-800 dark:text-zinc-200">
                            </flux:table.cell>

                            <flux:table.cell class="px-6 py-4">
                                <select
                                    class="w-full bg-transparent border-none text-sm focus:ring-2 focus:ring-red-500/20 rounded text-red-600 font-bold">
                                    <option selected value="1">Backend Development 2 (SWD413)</option>
                                </select>
                            </flux:table.cell>

                            <flux:table.cell class="px-6 py-4">
                                <input type="text" value="DUPLICATE ENTRY DETECTED" readonly
                                    class="w-full bg-transparent border-none focus:ring-2 focus:ring-red-500/20 rounded px-2 py-1 text-sm text-red-600 font-bold italic">
                            </flux:table.cell>

                            <flux:table.cell class="px-6 py-4">
                                <select
                                    class="w-full bg-transparent border-none text-sm focus:ring-2 focus:ring-red-500/20 rounded text-red-600 font-bold">
                                    <option selected value="1">First Semester</option>
                                </select>
                            </flux:table.cell>

                            <flux:table.cell align="center" class="px-6 py-4">
                                <div class="flex items-center justify-center gap-1">
                                    <flux:button variant="ghost" icon="exclamation-triangle" size="sm"
                                        class="text-red-500" />
                                    <flux:button variant="ghost" color="red" icon="x-mark" size="sm" />
                                </div>
                            </flux:table.cell>
                        </flux:table.row>
                    </flux:table.rows>
                </flux:table>
            </div>

            <div
                class="p-4 bg-zinc-100/50 dark:bg-white/5 border-t border-zinc-200/50 dark:border-white/5 flex justify-between items-center text-xs text-zinc-500">
                <p>Showing 2 registrations in queue</p>
                <p class="flex items-center gap-2">
                    <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-green-500"></span> 1
                        Clean Mapping</span>
                    <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-red-500"></span> 1
                        Unique Constraint Error</span>
                </p>
            </div>
        </flux:card>

        <flux:card class="space-y-4">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <flux:heading size="lg">Active Course Registrations Log</flux:heading>
                <div class="w-full md:w-80">
                    <flux:input icon="magnifying-glass" placeholder="Search by student details or course code..."
                        size="sm" />
                </div>
            </div>

            <flux:table>
                <flux:table.columns>
                    <flux:table.column>Student Name / Matric</flux:table.column>
                    <flux:table.column>Course Code & Title</flux:table.column>
                    <flux:table.column>Academic Session</flux:table.column>
                    <flux:table.column>Semester</flux:table.column>
                    <flux:table.column>Authorized By</flux:table.column>
                    <flux:table.column align="end">Action</flux:table.column>
                </flux:table.columns>

                <flux:table.rows>
                    <flux:table.row>
                        <flux:table.cell>
                            <div class="font-medium text-zinc-800 dark:text-zinc-200">John Doe</div>
                            <div class="text-xs text-zinc-400 font-mono">CS/2023/001</div>
                        </flux:table.cell>
                        <flux:table.cell>
                            <span class="font-semibold text-emerald-600 dark:text-emerald-400">SWD413</span>
                            <span class="text-xs text-zinc-500 block">Backend Development 2</span>
                        </flux:table.cell>
                        <flux:table.cell>
                            <flux:badge size="sm" color="zinc" variant="flat">2025/2026</flux:badge>
                        </flux:table.cell>
                        <flux:table.cell>First Semester</flux:table.cell>
                        <flux:table.cell>
                            <span
                                class="text-xs bg-zinc-100 dark:bg-zinc-800 px-2 py-1 rounded text-zinc-600 dark:text-zinc-400 font-medium">
                                Admin (ID: #{{ auth()->id() ?? 1 }})
                            </span>
                        </flux:table.cell>
                        <flux:table.cell align="end">
                            <flux:button icon="pencil" variant="ghost" size="sm"
                                :href="route('edit-course-registration', [1])">
                                Edit
                            </flux:button>
                        </flux:table.cell>
                    </flux:table.row>
                </flux:table.rows>
            </flux:table>
        </flux:card>
    </div>
</x-layouts::app>
